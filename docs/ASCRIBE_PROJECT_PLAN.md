...
pip install fastapi uvicorn[standard] sqlalchemy[asyncio] asyncpg alembic \
  ...
  pgvector pytest pytest-asyncio httpx python-dotenv
```
...
# Install TanStack Router (replaces react-router-dom)
npm install @tanstack/react-router
npm install -D @tanstack/router-devtools @tanstack/router-vite-plugin

# Install Tailwind CSS V4 (CSS-first — no tailwind.config.ts needed)
...
import { routeTree } from './routeTree.gen'   // auto-generated by the Vite plugin
import './index.css'
...
- Pre-commit hooks via `pre-commit` to enforce both before every commit
